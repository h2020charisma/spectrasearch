...
RRUFF is one such external viewer: `study`/`substance` results whose `name` looks like an RRUFF sample id (e.g. `R250095`, from Solr `name_s`/`name_hs` — see `pipeline_nexus/tasks/read_rruff.py`) link to `https://www.rruff.net/{name}`. The mineral's human-readable name lives in `publicname_s`/`publicname_hs` instead, not in `name`.
